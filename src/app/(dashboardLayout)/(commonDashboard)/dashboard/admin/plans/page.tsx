...
import { Plus, CreditCard, Users, DollarSign, TrendingUp, Edit, Eye, Loader2, CheckCircle2, Trash2, Power } from "lucide-react";
...
export default function AdminPlansPage() {
  ...
  const totalMembers = plans.reduce((sum, plan) => sum + (plan._count?.members || 0), 0);
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">of {plans.length} total</p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative bg-white rounded-3xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 ${plan.isPopular ? "ring-2 ring-orange-500" : ""
                  }`}
              >
                {/* Popular Badge */}
                {plan.isPopular && (
                ...
                {/* Card Header */}
                <div className={`p-8 ${plan.isPopular ? "bg-gradient-to-br from-orange-50 to-red-50" : "bg-gray-50"}`}>
                  ...
                  <div className="flex items-baseline gap-2 mt-6">
                    ...
                    <span className="text-gray-600 text-lg">/{plan.durationDays} days</span>
                  </div>
                  {plan.discount > 0 && (
                    <div className="mt-2">
                      <span className="text-sm font-semibold text-green-600">{plan.discount}% OFF</span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {plan.personalTrainingSessions > 0 && (
                      <div className="flex items-start gap-3">
                        ...
                        <span className="text-sm text-gray-700">{plan.personalTrainingSessions} Personal Training Sessions</span>
                      </div>
                    )}
                  </div>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this plan? This action cannot be undone and will affect all members currently enrolled in this plan.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
